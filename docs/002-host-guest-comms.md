# 002: Talking to the guest over vsock and a tiny agent
...
This is the engine's first real *interaction* with the guest. Everything later needs a way to put
work in and get results out: a language runtime (Phase 3), a network policy the guest can actually
exercise (Phase 4), the eBPF flight recorder watching a real workload (Phase 8+). That way is this
chapter.
...
asserts `hi\n`, exit 0. The rest of the taxonomy each has its own test: a guest that drops mid-exec
(`Channel`), a command that can't spawn (`GuestExec`), a signal-killed command (a faithful
`RunResult`), an output flood (`OutputCap`), a guest timeout (`ExecTimeout`). Only the guest's
